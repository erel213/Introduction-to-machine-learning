...
class conditional_independence():
    ...
    def is_X_Y_dependent(self):
        ...
        ###########################################################################
        for event_x in X.keys():
            for event_y in Y.keys():
                if X[event_x] * Y[event_y] != X_Y[event_x,event_y]:
                    return False
        return True
        ###########################################################################
        #                             END OF YOUR CODE                            #
    ...
    def is_X_Y_given_C_independent(self):
        ...
        X_Y_C = self.X_Y_C
        ###########################################################################
        for event in self.X_Y_C.keys():
            event_x_c = X_C[event[0],event[2]]
            event_y_c = Y_C[event[1], event[2]]
            if not np.isclose(event_x_c * event_y_c, X_Y_C[event]):
                print("Event is {} and event X_C {} event Y_C {}".format(event, event_x_c, event_y_c))
                return False
...
class NaiveNormalClassDistribution():
    def __init__(self, dataset, class_value):
        ...
        self.standard_deviation = np.std(self.class_instances, axis=0)
...
class MultiNormalClassDistribution():
    ...
    def __init__(self, dataset, class_value):
        ...
        self.class_instances = dataset[dataset[:, -1] == class_value][:,:-1]
        self.mean = np.mean(self.class_instances, axis=0)
        self.cov_matrix = np.cov(self.class_instances, rowvar=False)
...
class MaxPrior():
    def __init__(self, ccd0 , ccd1):
        ...
        # TODO: Implement the function.                                           #
        ###########################################################################
        self.ccd0_prior = ccd0.get_prior()
        self.ccd1_prior = ccd1.get_prior()
        ###########################################################################
        #                             END OF YOUR CODE                            #
        ###########################################################################
    ...
    def predict(self, x):
        ...
        ###########################################################################
        ###########################################################################
        pred = 0 if self.ccd0_prior > self.ccd1_prior else 1
        ###########################################################################
        #                             END OF YOUR CODE                            #
        ###########################################################################
        return pred
